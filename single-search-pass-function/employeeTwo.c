...
//compare employee phone number function
static int compareEmployeePhoneNum(const void *targetPtr, PtrToConstEmployee tableValuePtr){
    return strcmp((char *) targetPtr, tableValuePtr->phone);
}
//compare employee salary function
static int compareEmployeeSalary(const void *targetPtr, PtrToConstEmployee tableValuePtr){
    //casting, dereferencing, and comparing variables
...
//Passed thru function performing phone number search
PtrToEmployee searchEmployeeByPhoneNum(PtrToConstEmployee ptr, int size, char* phoneNum){
    return searchEmployeeTable(ptr, size, phoneNum, compareEmployeePhoneNum);
}

//Passed thru function performing salary search
